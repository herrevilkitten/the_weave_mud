...
bool run_olc_editor( DESCRIPTOR_DATA *d, char *argument )
{
    if ( !d || !d->character )
	return FALSE;
...
/*  {   command		function	}, */
    ...
    {   "gold",         medit_gold      },  /* ROM */
    {	"guild",	medit_guild	},  /* Eltan */
    {	"max",		medit_max	},  /* Eltan */
    {	"mcopy",	medit_mcopy	},  /* Eltan */
...
void aedit( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if ( LOWER(command[0]) == LOWER(aedit_table[cmd].name[0])
...
void redit( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if ( LOWER(command[0]) == LOWER(redit_table[cmd].name[0])
...
/*  int  value;   ROM */
    ...
    {
	if ( LOWER(command[0]) == LOWER(oedit_table[cmd].name[0])
...
/*  int  value;    ROM */
    ...
    {
	if ( LOWER(command[0]) == LOWER(medit_table[cmd].name[0]) 
...
void do_olc( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "You are not authorized to edit.\n\r", ch );
	return;
    }
...
void do_ae_olc( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    buf[0] = '\0';
    strcpy( buf, "area ");
...
void do_oe_olc( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    buf[0] = '\0';
    strcpy( buf, "obj "); 
...
void do_he_olc( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    buf[0] = '\0';
    strcpy( buf, "help "); 
...
void do_me_olc( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    buf[0] = '\0';
    strcpy( buf, "mob "); 
...
void do_ge_olc( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    buf[0] = '\0';
    strcpy( buf, "guild "); 
...
void do_re_olc( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    buf[0] = '\0';
    strcpy( buf, "room ");
...
void do_resets( CHAR_DATA *ch, char *argument )
{
	...
	if ( (!str_cmp( arg2, "mob" ) && is_number( arg3 ))
	  ...
	  || (!str_cmp( arg2, "aff" ) && is_number( arg3 )) )
	{
	    /*
		...
		{
/*		    pReset           = new_reset_data();  Why is here*/
		    pReset->command  = 'O';
		    pReset->arg2     = 1;
		    pReset->arg3     = ch->in_room->vnum;
		}
...
/*		    pReset = new_reset_data();  Why here? */
		    pReset->arg2 = 1;
	...
	send_to_char( "        RESET <number> AFF <type> <duration>\n\r", ch );
...
void do_alist( CHAR_DATA *ch, char *argument )
{
    ...
    for ( pArea = area_first; pArea; pArea = pArea->next )
    {
	sprintf( buf, "[%3d] %c%-27.27s%c (%-5d-%5d) %-12.12s [%d] [%-10.10s]\n\r",
	    pArea->vnum,
	    !IS_SET(pArea->area_flags, AREA_COMPLETE) ? '*' : ' ',
	    &pArea->name[8],
	    !IS_SET(pArea->area_flags, AREA_COMPLETE) ? '*' : ' ',
	    pArea->lvnum,
	    pArea->uvnum,
