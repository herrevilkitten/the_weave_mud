...
struct hash_header
{
    ...
    int                   *keylist, klistsize, klistlen;
			  /* this is really lame, AMAZINGLY lame */
                  ...
                  if( tmp_room != out_room_vnum )
                    {
                      ...
                      if( !hash_find( &x_room, tmp_room )
                         && ( count < depth ) )
                        {
		          if ( herep->area != herep->exit[i]->u1.to_room->area )
			  {
			    area_count++;
			    if ( !check_skill(ch, gsn_hunt, 100 - area_count * 5 / 3, FALSE) )
			    {
...
void do_hunt( CHAR_DATA *ch, char *argument )
{
  ...
  if( ch->in_room == victim->in_room )
    {
      act( "$N is in this room!", ch, NULL, victim, TO_CHAR );
      return;
    }

  /*
   * Deduct some movement. -- Gimme a break
 ...
 */
/*  act( "$n carefully sniffs the air.", ch, NULL, NULL, TO_ROOM ); no room spamming */
   ...
   */
  if( !check_skill(ch, gsn_hunt, 95, TRUE) )
    {
      do
        {
   ...
   */
  sprintf( buf, "You guess that $N is %s from here.", dir_name[direction] );
...
void hunt_victim( CHAR_DATA *ch )
{
  ...
  if( ch->in_room == ch->hunting->in_room )
    {
	if ( IS_SENTIENT(ch) )
	    act( "$n glare$% at $N and say$%, 'Ye shall DIE!'",
		ch, NULL, ch->hunting, TO_ALL );
	else
	    act( "$n look$% at $N meanly and suddenly attack$% $M!",
		ch, NULL, ch->hunting, TO_ALL );
...
void report_dir( CHAR_DATA *ch, int dir_count[6] )
{
    int total_dir[6];
    int i;
    int nFound = 0;
    ...
    total_dir[0] = dir_count[0] - dir_count[2];
    total_dir[1] = dir_count[1] - dir_count[3];
    total_dir[2] = dir_count[2] - dir_count[0];
    total_dir[3] = dir_count[3] - dir_count[1];
    total_dir[4] = dir_count[4] - dir_count[5];
    total_dir[5] = dir_count[5] - dir_count[4];

    if ( IS_IMMORTAL(ch) )
    {
	send_to_char_new( ch,
	"N:%d[%d] E:%d[%d] S:%d[%d] W:%d[%d] U:%d[%d] D:%d[%d]\n\r",
	dir_count[0], total_dir[0],
	dir_count[1], total_dir[1],
	dir_count[2], total_dir[2],
	dir_count[3], total_dir[3],
	dir_count[4], total_dir[4],
	dir_count[5], total_dir[5] );
    }
    ...
    for ( i = 0; i < 6; i++ )
    {
        if ( total_dir[i] > 0 )
        {
			...
			(i==5 || i==6) ? "wards" : "" );
                ...
                case 1:
                    nFound++;
                    sprintf( buf2, "%s%s", dir_name[i],
			(i==5 || i==6) ? "wards" : "" );
                    break;
                case 2:
                    sprintf( buf3, ", and %s%s.", dir_name[i],
			(i==5 || i==6) ? "wards" : "" );
                    break;
            }
        }
