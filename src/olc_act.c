...
const struct olc_help_type help_table[] =
{
    ...
    {	"spec-mob",	spec_table,	 "Available special programs for mobs." 	 },
    {	"spec-obj",	spec_obj_table,	 "Available special programs for objects." 	 },
    {	"use",		use_fun_table,	 "Available use functions for objects." 	 },
    {	"sex",		sex_flags,	 "Sexes."			 },
    ...
    {	"apply",	apply_flags,	 "Applies for addaffect."	 },
    {	"furniture",	furniture_flags, "Furniture flags."		 },
    {	"ingredients",	ingredient_flags,"Ingredient types."		},
    {	"fruit",	fruit_flags,	"Fruit types.",			},
...
/* ROM specific bits: */
    ...
    {	"material",	material_type,	"Material obj is made from."    },
    {   "wclass",       weapon_class,	"Weapon class."                 }, 
    {   "wtype",        weapon_type,	"Special weapon type."          },
    {	"armor",	armor_flags,	"Wear locations for armor"	},
    {	"climate",	climate_flags,	"Climate types"			},
    {	"weather",	weather_flags,	"Terrain types"			},
    {	"terrain",	terrain_flags,	"Weather types"			},
    {	"",		0,		 ""				}
};
...
void show_spec_obj_cmds( CHAR_DATA *ch )
{
    ...
    col = 0;
    send_to_char( "Preceed special functions with 'spec_'\n\r\n\r", ch );
    for (spec = 0; spec_obj_table[spec].spec_fun != NULL; spec++)
    {
...
/*****************************************************************************
 Name:		show_use_fun_cmds
 Purpose:	Displays settable special functions for objects.
 Called by:	show_help(olc_act.c).
 ****************************************************************************/
void show_use_fun_cmds( CHAR_DATA *ch )
{
    char buf  [ MAX_STRING_LENGTH ];
    char buf1 [ MAX_STRING_LENGTH ];
    int  spec;
    ...
    buf1[0] = '\0';
    col = 0;
    send_to_char( "Preceed use functions with 'use_'\n\r\n\r", ch );
    for (spec = 0; use_fun_table[spec].use_fun != NULL; spec++)
    {
	sprintf( buf, "%-19.18s", &use_fun_table[spec].use_name[4] );
	strcat( buf1, buf );
	if ( ++col % 4 == 0 )
	    strcat( buf1, "\n\r" );
    }
...
bool show_help( CHAR_DATA *ch, char *argument )
{
          ...
          && !str_prefix( arg, help_table[cnt].command ) )
	{
	    ...
	    if ( help_table[cnt].structure == spec_obj_table )
	    {
		show_spec_obj_cmds( ch );
		return FALSE;
	    }
	    else
	    if ( help_table[cnt].structure == use_fun_table )
	    {
		show_use_fun_cmds( ch );
		return FALSE;
	    }
	    else
...
AEDIT( aedit_climate )
{
    ...
    if ( ( value = flag_value( climate_flags, argument ) ) != NO_FLAG )
    {
	pArea->climate = value;
	send_to_char( "Climate type for area set.\n\r", ch );
	return TRUE;
    }

    send_to_char( "That is not a type of climate.\n\r", ch );
    send_to_char( "? CLIMATE  for a list of climates\n\r", ch );
    return TRUE;
}
...
AEDIT( aedit_weather )
{
    ...
    {
	pArea->weather = value;
	send_to_char( "Weather type for area set.\n\r", ch );
	return TRUE;
    }

    send_to_char( "That is not a type of weather.\n\r", ch );
    send_to_char( "? WEATHER  for a list of weather\n\r", ch );
...
AEDIT( aedit_terrain )
{
    ...
    if ( ( value = flag_value( terrain_flags, argument ) ) != NO_FLAG )
    {
	pArea->terrain = value;
	send_to_char( "Terrain type for area set.\n\r", ch );
	return TRUE;
    }

    send_to_char( "That is not a type of terrain.\n\r", ch );
    send_to_char( "? TERRAIN  for a list of terrains\n\r", ch );
...
AEDIT( aedit_format )
{
    ...
    send_to_char( "All room descriptions in area formated.\n\r", ch );
    return TRUE;
}

AEDIT( aedit_delete )
{
    ROOM_INDEX_DATA *pRoom, *iRoom;
    ROOM_INDEX_DATA *prevRoom;
    AREA_DATA *pArea;
    int vnum, i, iHash;
    char arg[MAX_INPUT_LENGTH];
    bool found = FALSE;
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Delete which vnum?\n\r", ch );
	return FALSE;
    }
    ...
    while ( pRoom->people != NULL )
    {
	CHAR_DATA *vch;
	vch = pRoom->people;

	char_from_room( vch );
	char_to_room( vch, get_room_index(504) );
    }
	...
	if ( pRoom->exit[i] != NULL )
	{
	    ...
	    rev = rev_dir[i];
	    pToRoom = pRoom->exit[i]->u1.to_room;
    ...
    {
	prevRoom = NULL;
        for( iRoom = room_index_hash[iHash]; iRoom; iRoom = iRoom->next )
        {
	    if ( pRoom == iRoom )
	    {
		if ( prevRoom == NULL )
		{
		    found = TRUE;
		    room_index_hash[iHash] = iRoom->next;
		    break;
		}
		else
		{
		    found = TRUE;
		    prevRoom->next = iRoom->next;
		    break;
		}
	    }
	    prevRoom = iRoom;
...
bool change_exit( CHAR_DATA *ch, char *argument, int door )
{
     ...
     * ----------------------------------------
     */
    if ( ( value = flag_value( exit_flags, orig_arg ) ) != NO_FLAG )
    {
	ROOM_INDEX_DATA *pToRoom;
	sh_int rev;                                    /* ROM OLC */
	...
	if ( pToRoom == NULL )
	{
	    send_to_char( "That exit does not lead anywhere.\n\r", ch );
	    TOGGLE_BIT(pRoom->exit[door]->rs_flags,  value);
	    return FALSE;
	}
...
REDIT( redit_name )
{
    ...
    if ( has_color(argument) )
    {
	send_to_char( "Room names may NOT have color.\n\r", ch );
	return FALSE;
    }
...
REDIT( redit_rcopy )
{
    ...
    {
        send_to_char( "Syntax:  rcopy [vnum]\n\r", ch );
        return FALSE;
    }

    vnum = atoi( argument );

    if ( (copyRoom = get_room_index( vnum )) == NULL )
    {
	send_to_char( "That vnum is invalid.\n\r", ch );
	return FALSE;
    }

    free_string( pRoom->name );
    pRoom->name             =   str_dup( copyRoom->name );
    free_string( pRoom->description );
    pRoom->description      =   str_dup( copyRoom->description );
    pRoom->room_flags       =   copyRoom->room_flags;
    pRoom->sector_type      =   copyRoom->sector_type;
...
void show_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *obj )
{
	...
	case ITEM_FRUIT_TREE:
	    sprintf( buf,
		"[v0] Type:	       [%s]\n\r"
		"[v1] Chance of fruit: %d\n\r",
		flag_string(fruit_flags, obj->value[0]),
		obj->value[1] );
	    send_to_char( buf, ch );
	    break;
	case ITEM_POTION:
		...
		"[v1] Maximum Dosage:  %d\n\r"
	        "[v3] Poisoned:     %s\n\r",
		obj->value[0] != 0 ? potion_table[obj->value[1]].name
				    : "none",
		obj->value[1],
	        obj->value[3] != 0 ? "Yes" : "No" );
	    send_to_char( buf, ch );
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
	case ITEM_WEAPON:
	    ...
	    send_to_char( buf, ch );
 	    sprintf( buf, "[v5] To-hit bonus:   [%d]\n\r",
		     obj->value[5] );
	    send_to_char( buf, ch );
 	    sprintf( buf, "[v6] Damage bonus:   [%d]\n\r",
		     obj->value[6] );
	    send_to_char( buf, ch );
	    break;
...
bool set_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *pObj, int value_num, char *argument)
{
    switch( pObj->item_type )
    {
	    ...
	    switch ( value_num )
	    {
	        ...
	        case 2:
	            ...
	            break;
	        case 3:
	            send_to_char( "BRIGHTNESS OF LIGHT SET.\n\r\n\r", ch);
	...
	case ITEM_INGREDIENT:
		...
		default:
		    ...
		    return FALSE;
		case 0:
		    if ( (value = flag_value( ingredient_flags, argument )) != NO_FLAG )
		    {
			send_to_char( "INGREDIENT TYPE SET.\n\r\n\r", ch );
	...
	case ITEM_FRUIT_TREE:
		...
		int value;
	        default:
		    do_help( ch, "ITEM_FRUIT_TREE" );
	            return FALSE;
	        case 0:
		    if ( (value = flag_value( fruit_flags, argument )) != NO_FLAG )
		    {
			send_to_char( "FRUIT TYPE SET.\n\r\n\r", ch );
			pObj->value[0] = value;
		    }
		    else
		    {
		...
		case 1:
		    ...
		    ||   value > 50 )
		    {
			send_to_char( "The range of chance is 0 to 50%.\n\r", ch );
			return FALSE;
		    }		
 	    }
	    break;
		...
		int value;
		    ...
		    do_help( ch, "ITEM_SCROLL_POTION_PILL" );
	            return FALSE;
	        case 0:
		    if ( (value = potion_lookup( argument )) != 0 )
		    {
			send_to_char( "POTION TYPE SET.\n\r", ch );
			pObj->value[0] = value;
		    }
		    else
		    {
			...
			return FALSE;
	            ...
	            break;
	        case 1:
	            send_to_char( "MAXIMUM DOSAGE SET.\n\r\n\r", ch );
...
/* ARMOR for ROM: */
	    ...
	    if ( pObj->material == 0 )
	    {
		send_to_char( "Armor must have a material set.\n\r", ch );
		return FALSE;
	    }
	    else
	    {
		...
		min = break_table[pObj->material].armor_min;
		max = break_table[pObj->material].armor_max;
		num = atoi( argument );

		if ( value_num == 3 )
		{
		    min -= UMAX(0, break_table[pObj->material].fire_mod);
		    max -= UMAX(0, break_table[pObj->material].fire_mod);
		}
		if ( value_num == 4 )
		{
		    min -= UMAX(0, break_table[pObj->material].cold_mod);
		    max -= UMAX(0, break_table[pObj->material].cold_mod);
		}
		if ( value_num == 5 )
		{
		    min -= UMAX(0, break_table[pObj->material].lightning_mod);
		    max -= UMAX(0, break_table[pObj->material].lightning_mod);
		}

		if ( value_num < 6
		&&   (num < min || num > max) )
		{
		    send_to_char( "Armor value is outside range for material.\n\r", ch );
		    sprintf( buf, "Range for %s for this type of damage is %d to %d.\n\r",
			material_name(pObj->material),
			min,
			max );
		    send_to_char( buf, ch );
		    return FALSE;
		...
		{
		    send_to_char( "Armor value is outside range for material.\n\r", ch );
		    sprintf( buf, "EXOTIC range for %s is 0 to %d.\n\r",
			material_name(pObj->material),
			max/2 );
		    send_to_char( buf, ch );
		    return FALSE;
		...
		int value;
		    ...
		    break;
	        case 3:
		    send_to_char( "AC FIRE SET.\n\r\n\r", ch );
		    pObj->value[3] = atoi( argument );
		    break;
	        case 4:
		    send_to_char( "AC COLD SET.\n\r\n\r", ch );
		    pObj->value[4] = atoi( argument );
		    break;
	        case 5:
		    send_to_char( "AC ELECTRICITY SET.\n\r\n\r", ch );
		...
		case 7:
		    ...
		    {
			send_to_char( "PLACE FLAGS SET.\n\r\n\r", ch );
			TOGGLE_BIT(pObj->value[4], flag_value( armor_flags, argument) );
		    }
		    else
		    {
			do_help ( ch, "ITEM_ARMOR" );
...
/* WEAPONS changed in ROM */
		...
		int value;
		    ...
		    if ( is_guild_eq(pObj->vnum)
		    &&   value != 5 )
		    {
			send_to_char( "Guild weapons must be 5d10.\n\r", ch );
			return FALSE;
		    }
		    else if ( !IS_SET(pObj->extra_flags, ITEM_QUEST)
		    &&        value != 5 )
		    {
			if ( value < 1
			||   value > 3 )
			{
			    send_to_char( "Only weapons marked as QUEST may have more than 3 dice.\n\r", ch );
			    return FALSE;
			}
		    }
		    ...
		    else if ( !is_guild_eq(pObj->vnum) )
		    {
			if ( value != weapon_table[pObj->value[0]].dice )
			{
			    send_to_char_new( ch, "The dice value must be %d for this type of weapon.\n\r",
				weapon_table[pObj->value[0]].dice );
			    return FALSE;
			}
		    }
	        ...
	        case 3:
	            ...
	            break;
	        case 4:
		    if ( (value = flag_value( weapon_type, argument )) != NO_FLAG )
		    {
			send_to_char( "SPECIAL WEAPON TYPE SET.\n\r\n\r", ch );
			TOGGLE_BIT(pObj->value[4], flag_value( weapon_type, argument) );
		    }
		    else
		    {
			do_help ( ch, "ITEM_WEAPON" );
		...
		case 5:
		    value = atoi( argument );
		    if ( value < -weapon_table[pObj->value[0]].max_tohit
		    ||   value >  weapon_table[pObj->value[0]].max_tohit )
		    {
			send_to_char_new( ch,
			    "The range for to-hit bonus is %d to %d.\n\r",
			    -weapon_table[pObj->value[0]].max_tohit,
			     weapon_table[pObj->value[0]].max_tohit );
			return FALSE;
		    }
		    pObj->value[5] = value;
		    send_to_char( "TO-HIT BONUS SET.\n\r\n\r", ch );
		    break;
		case 6:
		    value = atoi( argument );
		    if ( value < -weapon_table[pObj->value[0]].max_dam
		    ||   value >  weapon_table[pObj->value[0]].max_dam )
		    {
			send_to_char_new( ch,
			    "The range for to-hit bonus is %d to %d.\n\r",
			    -weapon_table[pObj->value[0]].max_dam,
			     weapon_table[pObj->value[0]].max_dam );
			return FALSE;
		    }
		    pObj->value[6] = value;
		    send_to_char( "DAMAGE BONUS SET.\n\r\n\r", ch );
		    break;
	    }
            break;
	...
	case ITEM_MONEY:
		...
		int value;
		    ...
		    do_help( ch, "ITEM_FURNITURE" );
	            return FALSE;
	        case 0:
		    send_to_char( "MAXIMUM PEOPLE SET.\n\r\n\r", ch );
		    pObj->value[0] = atoi( argument );
		    break;
	        case 1:
		    ...
		    {
			send_to_char( "SLEEP/REST/SIT/STAND FLAG SET.\n\r\n\r", ch );
...
OEDIT( oedit_show )
{
    ...
    send_to_char( buf, ch );
    sprintf( buf, "Quest Points:[%5d]\n\r",	
	pObj->qp );
    send_to_char( buf, ch );
    if ( pObj->spec_fun )
    {
	sprintf( buf, "Spec fun:    [%s]\n\r",  spec_obj_string( pObj->spec_fun ) );
	send_to_char( buf, ch );
    }
    if ( pObj->use_fun )
    {
	sprintf( buf, "Use fun:     [%s]\n\r",  use_fun_string( pObj->use_fun ) );
	send_to_char( buf, ch );
    }
...
OEDIT( oedit_qp )
{
    ...
    pObj->qp = atoi( argument );

    send_to_char( "Quest point cost set.\n\r", ch);
    return TRUE;
}
...
OEDIT( oedit_use )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Syntax:  use [use function]\n\r", ch );
	return FALSE;
    }
    ...
    if ( !str_cmp( argument, "none" ) )
    {
        ...
        send_to_char( "Use removed.\n\r", ch);
        return TRUE;
    }

    if ( use_fun_lookup( argument ) )
    {
	pObj->use_fun = use_fun_lookup( argument );
	send_to_char( "Use set.\n\r", ch);
	return TRUE;
    }

    send_to_char( "OEdit: No such use function.\n\r", ch );
    return FALSE;
}
...
OEDIT( oedit_max )
{
	...
	pObj->max = value;
	send_to_char( "Maximum set.\n\r", ch );

	return TRUE;
    }

    send_to_char( "Syntax:  max [number]\n\r"
		  "Where number can range from 0 (no limit) to 50.\n\r",
		  ch );
    return FALSE;
}
...
OEDIT( oedit_ocopy )
{
    ...
    if ( (copyObj = get_obj_index( vnum )) == NULL )
    {
        send_to_char( "That vnum is invalid.\n\r", ch );
        return FALSE;
    }
    ...
    free_string( pObj->name );
    pObj->name          =   str_dup( copyObj->name );
    free_string( pObj->short_descr );
    pObj->short_descr   =   str_dup( copyObj->short_descr );
    free_string( pObj->description );
    pObj->description   =   str_dup( copyObj->description );
    pObj->item_type     =   copyObj->item_type;
    pObj->extra_flags   =   copyObj->extra_flags;
    pObj->wear_flags    =   copyObj->wear_flags;
    pObj->weight        =   copyObj->weight;
    pObj->cost          =   copyObj->cost;
    pObj->material      =   copyObj->material;      /* ROM */
    pObj->condition     =   100;                        /* ROM */
    for ( value = 0; value < 5; value++ )               /* 5 - ROM */
        pObj->value[value]  =   copyObj->value[value];
    pObj->max           =   copyObj->max;
...
MEDIT( medit_guild )
{
    ...
    send_to_char( "Syntax:  guild [guild name]\n\r", ch );
    return FALSE;
}
...
MEDIT( medit_max )
{
   ...
   {
	send_to_char( "Range is from 0 to 50.\n\r", ch );
	return FALSE;
    }

    pMob->max = atoi( argument );
...
MEDIT( medit_mcopy )
{
    ...
    if ( (copyMob = get_mob_index( vnum )) == NULL )
    {
	send_to_char( "That vnum is invalid.\n\r", ch );
	return FALSE;
    }
    ...
    pMobIndex->player_name          = str_dup( copyMob->player_name );
    free_string( pMobIndex->short_descr );
    pMobIndex->short_descr          = str_dup( copyMob->short_descr );
    free_string( pMobIndex->long_descr  );
    pMobIndex->long_descr           = str_dup( copyMob->long_descr );
    free_string( pMobIndex->description );
    pMobIndex->description          = str_dup( copyMob->description );
    pMobIndex->race                 = copyMob->race;

    pMobIndex->long_descr[0]        = UPPER(pMobIndex->long_descr[0]);
    pMobIndex->description[0]       = UPPER(pMobIndex->description[0]);

    pMobIndex->act                  = copyMob->act | ACT_IS_NPC;
    pMobIndex->affected_by          = copyMob->affected_by;
    if ( copyMob->pShop != NULL )
    {
	if ( !pMobIndex->pShop )
	{
	    pMobIndex->pShop		= new_shop();
	    pMobIndex->pShop->keeper	= pMobIndex->vnum;
	    shop_last->next		= pMobIndex->pShop;
	}
            
	pMobIndex->pShop->open_hour	= copyMob->pShop->open_hour;
	pMobIndex->pShop->close_hour	= copyMob->pShop->close_hour;
	pMobIndex->pShop->profit_buy	= copyMob->pShop->profit_buy;
	pMobIndex->pShop->profit_sell	= copyMob->pShop->profit_sell;
	for ( i = 0; i < MAX_TRADE; i++ )
	    pMobIndex->pShop->buy_type[i] = copyMob->pShop->buy_type[i];
    }

    pMobIndex->level                = copyMob->level;
    pMobIndex->hitroll              = copyMob->hitroll;

    pMobIndex->hit[DICE_NUMBER]     = copyMob->hit[DICE_NUMBER];
    pMobIndex->hit[DICE_TYPE]       = copyMob->hit[DICE_TYPE];
    pMobIndex->hit[DICE_BONUS]      = copyMob->hit[DICE_BONUS];

    pMobIndex->damage[DICE_NUMBER]  = copyMob->damage[DICE_NUMBER];
    pMobIndex->damage[DICE_TYPE]    = copyMob->damage[DICE_TYPE];
    pMobIndex->dam_type             = copyMob->dam_type;

    pMobIndex->ac[AC_PIERCE]        = copyMob->ac[AC_PIERCE];
    pMobIndex->ac[AC_BASH]          = copyMob->ac[AC_BASH];
    pMobIndex->ac[AC_SLASH]         = copyMob->ac[AC_SLASH];
    pMobIndex->ac[AC_EXOTIC]        = copyMob->ac[AC_EXOTIC];

    pMobIndex->off_flags            = copyMob->off_flags;
    pMobIndex->imm_flags            = copyMob->imm_flags;
    pMobIndex->res_flags            = copyMob->res_flags;
    pMobIndex->vuln_flags           = copyMob->vuln_flags;

    pMobIndex->start_pos            = copyMob->start_pos;
    pMobIndex->default_pos          = copyMob->default_pos;
    pMobIndex->sex                  = copyMob->sex;
    pMobIndex->gold                 = copyMob->gold;

    pMobIndex->form                 = copyMob->form;
    pMobIndex->parts                = copyMob->parts;
    pMobIndex->size		    = copyMob->size;
    pMobIndex->guild                = copyMob->guild;
    pMobIndex->material             = copyMob->material;
    pMobIndex->max                  = copyMob->max;
